...
class Cat
  ...
  def heal
   ...
   def alive
   	...
   	if @mood < 0
   	 'Ваш питомец сбежал от вас в поисках приколючений :('
   	elsif @eat < 0
   	 'Ваш питомец умер от голода :('
   	elsif @health < 0
   		msg = 'Ваш питомец умер от болезни :(' 
      msg += '...которую заработал потому что был грязнулей.' if @dirty > 6
      msg
    else
    	'OK'
    end
